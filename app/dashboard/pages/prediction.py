"""Prediction Interactive — render function for tab."""
...
from components.shared import (
    load_data, load_artifacts, LABELS, PL, C,
)
...
def render():
    ...
    with form_left:
        ...
        med_inc = st.number_input("Revenu median du quartier (x$10k)", min_value=0.5, max_value=15.0, value=3.87, step=0.1, format="%.2f", key="pred_inc")
        population = st.number_input("Population du district", min_value=3, max_value=35000, value=1425, step=50, key="pred_pop")
        ave_occup = st.number_input("Occupants par logement (moyenne)", min_value=0.5, max_value=10.0, value=3.07, step=0.1, format="%.2f", key="pred_occ")

    with form_right:
        st.markdown('<div class="section-label">Caracteristiques du logement</div>', unsafe_allow_html=True)
        house_age = st.number_input("Age median des logements (annees)", min_value=1, max_value=52, value=29, step=1, key="pred_age")
        ave_rooms = st.number_input("Nombre de pieces (moyenne)", min_value=1.0, max_value=15.0, value=5.43, step=0.1, format="%.2f", key="pred_rooms")
        ave_bedrms = st.number_input("Nombre de chambres (moyenne)", min_value=0.3, max_value=5.0, value=1.10, step=0.1, format="%.2f", key="pred_bed")
    ...
    # PREDICTION
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    if st.button("Estimer le prix", type="primary", key="pred_btn"):
